...
CreateThread(function()
	exports["esx_society"]:registerSociety('cardealervip', TranslateCap('car_dealer_vip'), 'society_cardealervip', 'society_cardealervip', 'society_cardealervip', {type = 'private'})
end)
...
RegisterNetEvent('esx_vehicleshopvip:setVehicleOwnedPlayerId')
AddEventHandler('esx_vehicleshopvip:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)

	if xPlayer.job.name ~= 'cardealervip' or not xTarget then
		return
	end

	MySQL.scalar('SELECT id FROM cardealer_vehiclesvip WHERE vehicle = ?', {model},
	function(id)
		if not id then
			return
		end

		MySQL.update('DELETE FROM cardealer_vehiclesvip WHERE id = ?', {id},
...
RegisterNetEvent('esx_vehicleshopvip:rentVehicle')
AddEventHandler('esx_vehicleshopvip:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
	...
	MySQL.single('SELECT id, price FROM cardealer_vehiclesvip WHERE vehicle = ?', {vehicle},
	function(result)
		if not result then
			return
		end

		MySQL.update('DELETE FROM cardealer_vehiclesvip WHERE id = ?', {result.id},
		function(rowsChanged)
			if rowsChanged ~= 1 then
				return
			end
...
RegisterNetEvent('esx_vehicleshopvip:getStockItem')
AddEventHandler('esx_vehicleshopvip:getStockItem', function(itemName, count)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)

	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealervip', function(inventory)
...
RegisterNetEvent('esx_vehicleshopvip:putStockItems')
...
ESX.RegisterServerCallback('esx_vehicleshopvip:getCommercialVehicles', function(source, cb)
	MySQL.query('SELECT price, vehicle FROM cardealer_vehiclesvip ORDER BY vehicle ASC', function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_vehicleshopvip:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_cardealervip', function(account)
		...
		MySQL.insert('INSERT INTO cardealer_vehiclesvip (vehicle, price) VALUES (?, ?)', {model, modelPrice},
		function(rowsChanged)
			cb(true)
		end)
	end)
...
RegisterNetEvent('esx_vehicleshopvip:returnProvider')
AddEventHandler('esx_vehicleshopvip:returnProvider', function(vehicleModel)
	...
	MySQL.single('SELECT id, price FROM cardealer_vehiclesvip WHERE vehicle = ?', {vehicleModel},
	function(result)
		if not result then
...
ESX.RegisterServerCallback('esx_vehicleshopvip:giveBackVehicle', function(source, cb, plate)
	...
	function(result)
		...
		function()
			MySQL.insert('INSERT INTO cardealer_vehiclesvip (vehicle, price) VALUES (?, ?)', {result.vehicle, result.base_price})
...
ESX.RegisterServerCallback('esx_vehicleshopvip:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealervip' or not Config.EnablePlayerManagement then
