...
function PlayerManagement()
	...
	if ESX.PlayerData.job.name ~= 'cardealervip' then
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_vehicleshopvip:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_vehicleshopvip:returnProvider', data.current.value)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_vehicleshopvip:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
...
function OpenResellerMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elements = {
			...
			{label = TranslateCap('get_rented_vehiclesvip'),            value = 'get_rented_vehiclesvip'},
		...
		if Config.OxInventory and (action == 'put_stock' or action == 'get_stock') then
			exports.ox_inventory:openInventory('stash', 'society_cardealervip')
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealervip', TranslateCap('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'get_rented_vehiclesvip' then
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('esx_vehicleshopvip:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								...
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('esx_vehicleshopvip:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								ESX.ShowNotification(TranslateCap('no_players'))
							end
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_vehicleshopvip:getRentedVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'rented_vehiclesvip', {
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'cardealervip', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('esx_vehicleshopvip:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_vehicleshopvip:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_vehicleshopvip:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_vehicleshopvip:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_vehicleshopvip:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
function hasEnteredMarker(zone)
	if zone == 'ShopEntering' then
		...
		if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealervip' then
			CurrentAction     = 'reseller_menu'
			CurrentActionMsg  = TranslateCap('shop_menu')
			CurrentActionData = {}
		end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealervip' and ESX.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = TranslateCap('shop_menu')
		CurrentActionData = {}
	end
...
if Config.Blip.show then
	CreateThread(function()
		...
		AddTextComponentSubstringPlayerName('Concesionario ~y~VIP')
...
--[[Citizen.CreateThread(function()
	...
	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName('Concesionario')
	EndTextCommandSetBlipName(blip)
end)]]--
...
CreateThread(function()
	while true do
		...
		if CurrentAction then
			ESX.TextUI(CurrentActionMsg) -- or ESX.ShowHelpNotification(CurrentActionMsg)
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleshopvip:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleshopvip:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
							ESX.ShowNotification(TranslateCap('vehicle_soldvip_for', CurrentActionData.label, ESX.Math.GroupDigits(CurrentActionData.price)))
						else
